            ...
            <div className="flex flex-col items-center">
                <div className="max-w-4xl w-full relative mx-auto p-6 rounded">
                    ...
                    {editable && <div className=' bg-green-600 hover:bg-amber-600 p-3 rounded-full drop-shadow-md absolute sm:bottom-6 -bottom-10 right-6 cursor-pointer'>
                        ...
                        <input type="file" id="coverimageInput" className='hidden' accept='image/*' onChange={(e) => handleCoverImageChange(e.target)} />
                    </div>}
                    ...
                    <div className="w-full">
                        ...
                        <form className="grid grid-cols-1 gap-6 mt-8 md:grid-cols-2" onSubmit={handleSubmit(create)} encType='multipart/form-data'>
                            <div>
                                ...
                                <input className={inputClassName} type="text" value={data?.fullname} readOnly={!editable} {...register("fullname")} />
                            </div>

                            <div>
                                <label className="block text-gray-700 text-sm font-bold mb-2">Email</label>
                                <input className={inputClassName} type="email" value={data?.email} readOnly={!editable} {...register("email")} />
                            </div>

                            <div>
                                <label className="block text-gray-700 text-sm font-bold mb-2">Username</label>
                                <input className={inputClassName} type="text" value={data?.username} readOnly={!editable} {...register("username")} />
                            </div>

                            {editable &&<div>
                                <label className="block text-gray-700 text-sm font-bold mb-2">New Password</label>
                                <input className={inputClassName} type="password" readOnly={!editable} {...register("password")} />
                            </div>}
                            {editable &&<div>
                                <label className="block text-gray-700 text-sm font-bold mb-2">Confirm Password</label>
                                <input className={inputClassName} type="password" readOnly={!editable} {...register("password")} /> </div>}
                            {editable && <button
